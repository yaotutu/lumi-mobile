    ...
    immer((set, get) => ({
      ...
      toggleLike: async (modelId: string, currentLikes: number) => {
        ...
        try {
          ...
          logger.info(`✅ 点赞状态切换成功: modelId=${modelId}, isLiked=${response.isInteracted}, likeCount=${response.likeCount}`);
      ...
      toggleFavorite: async (modelId: string, currentFavorites: number) => {
        ...
        try {
          ...
          logger.info(`✅ 收藏状态切换成功: modelId=${modelId}, isFavorited=${response.isInteracted}, favoriteCount=${response.favoriteCount}`);
          logger.debug(`📊 当前 statusMap[${modelId}]:`, get().statusMap[modelId]);
